...
module.exports = function mount(app, ctx) {
   ...
   *   ③ **레이트리밋**: 사용자당 1분 60회 — **부하 방지용이지 수집 방지가 아니다**. 50개씩
   *      9번이면 420문항이 다 나오고, 애초에 로그인 사용자는 채점 라우트(분당 20회 × 회차 20문항)로
   *      전부터 전 문항 정답을 받을 수 있었다. 로그인 사용자에게 정답을 여는 것은 요구 2 의
   *      의도된 결정이고, 이 관문이 막는 것은 익명 수집(①)과 대전 중 치팅(②)이다.
  ...
  app.get('/api/study/explain', auth.requireAuth, explainLimit, function (req, res) {
    ...
    // 카드 하나씩 여는 게 보통이라(문항당 1요청) 매 요청을 적으면 학습 한 판에 20줄이 쌓인다.
    // 일괄 조회와 대전 잠금이 걸린 경우만 남긴다.
    if (blocked > 0 || ids.length > 1) {
