    ...
    return api.get(peekPath() + '?ids=' + q).then(function (data) {
      ...
      ids.forEach(function (qid) {
        ...
        if (!row || typeof row !== 'object') {
          // 서버가 조용히 생략했다. 빈 값으로 캐시하면 "해설이 아직 없습니다" 처럼 보인다 —
          // 오답노트에서는 "채점 기록 없음" 이라 영구적이므로 권한 없음으로 기억해 두지만
          // (같은 요청을 되풀이하지 않도록), 학습·모의고사에서 생략되는 이유는 **대전 잠금뿐**이고
          // 그건 대전이 끝나면 풀린다 — 기억해 두면 새로고침 전까지 영영 못 연다(리뷰 2026-09-08).
          if (state.mode === 'wrong') state.peekDenied[qid] = true;
          denied.push(qid);
          return;
        }
        state.peek[qid] = {
  ...
  function renderPeekBar() {
    ...
    node.appendChild(el('span', 'peek-hint', state.mode === 'wrong'
      ...
      : '학습 중에는 언제든 정답·해설을 펼쳐 볼 수 있습니다. (대전 중인 문항은 제외'
        + (state.meLoaded && !state.me ? ' · 로그인 필요' : '') + ')'));
  }
  ...
  function renderQuestion(question, index) {
    ...
    if (graded) {
      ...
      if (detail.near) {
        ...
        // 서술형 — 핵심어 일부만. "표기만 다름" 과는 다른 말이어야 한다(내용이 모자란 것이다).
        card.appendChild(el('p', 'feedback near',
          '핵심어를 일부만 맞혔습니다 — 위의 정답 표기에서 빠진 핵심어를 확인해 보세요.'));
      }
