...
| GET | `/api/me/wrong/explain` | `?ids=<qid,qid,…>` 1~50개 (로그인 필수) | `{explanations:{qid:{display,html,bodyText}}}` — **채점 전 비노출의 예외 ①(오답노트)**. 사용자가 이미 채점 기록을 가진 문항만 담고 나머지는 조용히 생략한다(403 아님). **지금 진행 중인 대전에 걸린 문항도 같은 규칙으로 생략한다**(전부 잠기면 빈 맵 200). 비로그인 401 / ids 가 비었거나 50개 초과면 400 |
| GET | `/api/study/explain` | `?ids=<qid,qid,…>` 1~50개 (로그인 필수) | `{explanations:{qid:{display,html,bodyText}}}` — **채점 전 비노출의 예외 ②(학습 모드)**. 채점 이력을 보지 **않는다** — 학습·모의고사 화면에서 모르는 문항의 정답·해설을 바로 펼쳐 보기 위한 경로다. **지금 진행 중인 대전에 걸린 문항**과 없는 문항 id 는 조용히 생략한다(403 아님). 비로그인 401 / ids 가 비었거나 50개 초과면 400 / 사용자당 분당 60회 초과 429 |
...
| POST | `/api/rooms` | `{name, mode:"round"\|"random", roundIds[], questionCount(5\|10\|20, random만), type?:"code"\|"sql"\|"theory", lang?:"c"\|"java"\|"python", timeLimitS(0\|600\|1200\|1800\|3600\|7200 — **0 은 제한 없음**), inviteUserIds?:number[](최대 8, 생성자·정수 아닌 값 무시)}` | `{roomId}` |
...
(`create(ctx) → {activeIds, blocks}`) 한 곳에만 두고 세 경로가 같은 객체를 쓴다 —
채점은 `blocks()` 로 집합을 통째로 거절(409)하고, `GET /api/me/wrong/explain` · `GET /api/study/explain` 은
`activeIds()` 로 잠긴 문항만 조용히 생략한다. **새로 정답·해설을 내보내는 경로를 추가하면 여기를 반드시 통과시켜라.**
...
- **예외 ①(오답노트)**: 이미 채점 기록이 있는 문항의 정답·해설은 오답노트에서 채점 전에도
  ...
  **조용히 생략**한다 — 403 을 주면 "그 문항이 존재하는가"가 새기 때문이다.
  (이 규칙은 아래 예외 ② 와 함께 읽는다.)
- **예외 ②(학습 모드, 2026-09-08 요구 2)**: 학습·모의고사 화면에서는 **채점 이력과 무관하게**
  `GET /api/study/explain` 으로 정답 표기·해설·지문 원문(`bodyText`, AI 질문 프롬프트 조립용)을
...
- **예외의 예외 — 대전 잠금**: "이미 채점받았으니 새로 새는 정보가 없다" 는 전제는 **대전 중에는
  ...
  **같은 잠금**을 통과한다 — `ctx.battleIo.activeBattleQuestionIds(userId)` 에 든 문항 id 는
  ...
  `routes/study.js`(채점 409 · `/api/study/explain` 생략)와 `routes/me.js`(오답노트 해설 생략)가
  같은 객체를 쓴다 — 사본을 두면 어긋난다.
...
| `battle:questions` | S→C | `{questions[](정답·sampleAnswer 제외, 각 문항에 `type`·`lang` 포함), deadlineInfo:{startedAt,deadline,timeLimitS,remainingMs}}` — countdown 종료 시. **제한 없음 방은 `deadline`·`remainingMs` 가 `null`, `timeLimitS` 가 0** |
