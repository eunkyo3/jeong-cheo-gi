...
- `normalize`
  ...
  - `keepSpace` — NFC → 보이지 않는 공백 정리 → 줄마다 trim + 연속 공백 1칸 압축 → `,;:()[]{}=` 주변 공백 제거.
    **공백의 유무와 대소문자는 유지**한다(`0 1 2 3` ≠ `0123`). **코드 출력 문항 전용.**
    2026-09-08(요구 3): 예전에는 `trim()` 뿐이라 `a = 10` 과 `a=10` 이 서로 달랐다 — 데이터가 표기 변형을
    accept 에 열거해 막던 것을 정규화가 흡수한다. 인정 범위를 넓히기만 하므로 기존 accept 는 그대로 유효하다.
    대소문자 무시를 유지하려면 accept 에 대/소문자 변형을 모두 나열한다.
  - `sql` — NFC → 보이지 않는 공백 정리 → 소문자 → 연속 공백 1칸 압축 → `,()=<>` 주변 공백 제거 → 후행 `;`/`.`/공백 제거.
    (`,()` 는 2026-09-04, `=<>` 는 2026-09-08 — `WHERE a = 1` ≡ `WHERE a=1`. accept 에 그 변형을 열거할 필요 없다.)
- `answerMode`
