...
- 대전 모드: 닉네임+비밀번호로 로그인 → 방 생성/참여 → 동시 시작 → 실시간 진행 현황 → 승자 판정 (제한 시간 **10분 / 20분 / 30분 / 1시간 / 2시간 / 제한 없음** — 제한 없음 방도 안전 상한 12시간이 지나면 자동 종료)
...
- 해설: **420문항 전 문항 해설** (비전공자 눈높이, 핵심 강조 표시). **학습·모의고사·오답노트에서는 채점 전에도** 문항마다 "정답·해설 보기" 토글과 "AI에게 질문하기" 를 쓸 수 있습니다 — 학습은 공부하는 자리라 모르면 바로 보고 이해하고 넘어갈 수 있어야 하기 때문입니다(로그인 필요). **대전 중인 문항만은 예외로 잠깁니다**. 대전 화면에서는 지금까지처럼 채점 후에만 열립니다
...
- 학습 하단 미니바(답한 n/N · 다음 빈칸으로 · 제출), 타이머는 접힌 상태 기본 — 타이머 없음 / **제한 없음(경과 시간만 표시, 자동 제출 없음)** / 30·60·90분 카운트다운(0이 되면 자동 제출)
...
- **라이트 / 다크 모드**: 상단 내비 오른쪽 끝의 ☀️/🌙 버튼으로 새로고침 없이 즉시 바꿉니다. 기본은 기기·브라우저의 밝기 설정을 따라가고, 버튼을 누를 때마다 자동 → 반대쪽 → 같은 쪽 고정 → 자동 순으로 돕니다(선택은 이 브라우저에 저장). 애니메이션 최소화 설정(`prefers-reduced-motion`)도 존중합니다
...
SQL 문항은 연속 공백을 한 칸으로 줄이고 **쉼표·괄호·비교 연산자 주변 공백**(`IN (3, 4)` ≡ `IN(3,4)`, `a = 1` ≡ `a=1`)과 후행 세미콜론을 무시합니다.
코드 출력 문항(`keepSpace`)은 **공백의 유무**와 대소문자를 그대로 비교합니다 — 출력값이 곧 정답이기 때문입니다.
다만 "몇 칸을 눌렀나"·"구두점 옆에 공백이 있나" 로는 틀리지 않습니다: 연속 공백은 한 칸으로 줄이고
`,` `;` `:` `(` `)` `[` `]` `{` `}` `=` 주변 공백을 무시합니다(`Vehicle name : Spark` ≡ `Vehicle name:Spark`, `[1, 2, 3]` ≡ `[1,2,3]`).

세 모드 모두 **눈에 보이지 않는 공백**(전각 공백, 비분리 공백, 폭 0 문자, BOM, 윈도우 줄바꿈)을 먼저 걷어냅니다 —
문제 지문이나 메신저에서 복사해 붙여 넣을 때 딸려 들어와 "화면상 정답인데 오답" 을 만들던 것들입니다.

**키워드만 맞으면 정답 처리하지는 않습니다.** 실기 실채점은 표기까지 보기 때문에, 여기서 조용히 정답을 주면
시험장에서도 통한다고 오해하게 됩니다. 대신 내용은 맞았는데 표기(대소문자·구두점)만 다른 답에는
**"거의 정답입니다 — 표기만 다릅니다(대소문자·구두점·띄어쓰기)"** 안내가 붙어, "몰라서 틀린 문항" 과 한눈에 구분됩니다(점수는 그대로 오답).
서술형 문항은 원래부터 `keywords` 검증기로 핵심어를 보고 채점하며, 핵심어를 일부만 맞힌 답에는 (표기 문제와는 다른 말로)
**"핵심어를 일부만 맞혔습니다"** 가 붙습니다.

채점 전 정답·해설 공개는 **로그인 사용자에게 열려 있는 것이 의도**입니다 — 조회 레이트리밋(분당 60회)은 부하 방지용이지
수집 방지가 아니며, 로그인 사용자는 이전부터 채점 경로로 전 문항 정답을 받을 수 있었습니다. 막는 것은 익명 수집과 **대전 중 치팅**입니다.
